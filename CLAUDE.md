...
| **B** | P450数据集构建与结构优化 | ✅ 已完成 | Step 1-10 全部完成 |
...
| **C** | P450专属模型训练 | 🔄 C2数据集构建启动 | `PathC_2026-03-19_P450专属模型训练/` |
| │    | └─ C1 模型架构优化 | 🔄 3/19-30 | C1-Step 1 ✅ → Step 2 dropout消融 ✅ (Val提升但**Test未迁移**) → 其他消融可与C2并行 |
| │    | └─ C2 P450全面数据集构建 | 🔄 3/22- | S1-S3+S8标准文件全部完成: S1(272对)+S2(806对)+S3(2798对)+S6(辅助)+S8(979对,578酶) ✅. 去重前4,855正样本, ~1,600+酶. 待: 其他数据库搜集→合并去重→负样本→Split |
