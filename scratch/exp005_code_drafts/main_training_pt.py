"""
EZSpecificity Training from .pt shard cache (PtCacheDataset).
...
Usage:
  # Fresh start:
  python main_training_pt.py --config train_pt_config.yml --cache-dir /path/to/pt_cache

  # Resume:
  python main_training_pt.py --config train_pt_config.yml --cache-dir /path/to/pt_cache --resume last
"""
...
EXPERIMENT_DIR = os.path.normpath(os.path.join(SCRIPT_DIR, ".."))

# Prefer experiment-local src/ for architecture isolation; fall back to global
_LOCAL_SRC = os.path.join(EXPERIMENT_DIR, "src")
_GLOBAL_SRC = os.path.normpath(os.path.join(EXPERIMENT_DIR, "..", "..", "..", "src"))
SRC_DIR = _LOCAL_SRC if os.path.isdir(_LOCAL_SRC) else _GLOBAL_SRC
...
from pt_dataset_dualgraph import PtCacheDualgraphDataset
from Models.ss_dualgraph import SSDualgraph     # EXP005 model (subclass of SS)
...
# Fast H2D transfer patch — applied only to SSDualgraph (EXP005 never
# instantiates raw SS, so no need to patch the parent).
def _fast_transfer(self, batch, device, dataloader_idx):
    return batch.to(device, non_blocking=True)


SSDualgraph.transfer_batch_to_device = _fast_transfer
...
def parse_args():
    ...
    p.add_argument("--cache-dir", required=True, help="Path to pt_cache directory")
    ...
    p.add_argument("--dropout", type=float, default=None,
                   help="Override cross-attention dropout (default: from config)")
    p.add_argument("--run-name", type=str, default=None,
                   help="Run tag for output naming (default: --edge-mode value)")
    p.add_argument("--test-only", action="store_true",
                   help="Run test evaluation only (no training)")
    p.add_argument("--checkpoint", type=str, default=None,
                   help="Checkpoint path for --test-only mode")
    p.add_argument("--output-json", type=str, default=None,
                   help="Output JSON path for --test-only results")
    return p.parse_args()
...
class PtTrainingDataModule(pl.LightningDataModule):
    ...
    def _make_dataset(self, split: str, is_train: bool) -> PtCacheDualgraphDataset:
        return PtCacheDualgraphDataset(
            ...
            preload_gvp=True,  # ~350 MB, always on for hot training
        )

    def _make_loader(self, split: str, is_train: bool, shuffle: bool = False):
        ds = self._make_dataset(split, is_train)
        kw = dict(
            batch_size=self.batch_size if is_train else min(self.batch_size * 3 // 2, 96), shuffle=shuffle,
            num_workers=self.num_workers, pin_memory=(self.num_workers > 0),
            # EXP005: follow_batch for BOTH baseline ligand_index and new GVP
            # node set. PyG auto-creates `batch.gvp_node_s_batch` which
            # SSDualgraph uses to map GVP nodes back to their sample id.
...
def plot_training_curves(csv_path: str):
    try:
        ...
        plt.suptitle("EZSpecificity Training Report", fontsize=16, fontweight="bold")
        plt.tight_layout()
...
# DDP Runtime Patches
# ---------------------------------------------------------------------------
def _apply_runtime_patches(model):
    """Apply DDP gather + nosync patches to model. Works for both train and test."""
    # Patch self.log: sync_dist=False (our gather handles it)
    _orig_log = model.log
    ...
    # PL 2.x: use on_*_epoch_end (no outputs arg) + manual caching + DDP gather
    if not hasattr(model, 'validation_step_outputs'):
        model.validation_step_outputs = []
    if not hasattr(model, 'test_step_outputs'):
        model.test_step_outputs = []

    def _wrap_epoch_end(attr_name, stage):
        def _ddp_epoch_end():
            import torch.distributed as dist
            outputs = list(getattr(model, attr_name))
            if dist.is_initialized() and dist.get_world_size() > 1:
                gathered = [None for _ in range(dist.get_world_size())]
                dist.all_gather_object(gathered, outputs)
                outputs = [item for rank_outputs in gathered for item in rank_outputs]
            if outputs:
                model.cutomized_epoch_end(outputs, stage)
            getattr(model, attr_name).clear()
        return _ddp_epoch_end

    model.on_validation_epoch_end = _wrap_epoch_end('validation_step_outputs', 'val')
    model.on_test_epoch_end = _wrap_epoch_end('test_step_outputs', 'test')
    return model
...
# Test Evaluation
# ---------------------------------------------------------------------------
def test_evaluate(args, config):
    """Run test-only evaluation on a checkpoint."""
    if not args.checkpoint:
        print("[ERROR] --test-only requires --checkpoint")
        sys.exit(1)

    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    batch_size = args.batch_size or config.data.batch_size
    run_tag = args.run_name or args.edge_mode

    # Output path
    if args.output_json:
        out_json = args.output_json
    else:
        out_json = os.path.join(RESULTS_DIR, "test_eval.json")

    print("=" * 70)
    print("Test Evaluation Mode")
    print(f"  Checkpoint : {args.checkpoint}")
    print(f"  Edge mode  : {args.edge_mode}")
    print(f"  Dropout    : {config.model.cross_attention.dropout}")
    print(f"  Batch size : {batch_size}")
    print(f"  Device     : {device}")
    print("=" * 70)

    # Dataset (EXP005: dualgraph)
    print("[1/3] Loading test dataset...")
    t0 = time.time()
    dataset = PtCacheDualgraphDataset(
        cache_dir=os.path.abspath(args.cache_dir),
        split="test", edge_mode=args.edge_mode, dist_noise=False,
    ...
    print(f"  {len(dataset)} samples, built in {time.time()-t0:.1f}s")

    # Model (EXP005 hard-assumes SSDualgraph — legacy SS checkpoints are NOT
    # compatible due to different specificity_header first-layer shape and
    # additional gvp_* parameters).
    print(f"[2/3] Loading checkpoint...")
    ckpt = torch.load(args.checkpoint, map_location="cpu", weights_only=False)
    model = SSDualgraph(config)
    sd = ckpt.get("state_dict", ckpt)
    result = model.load_state_dict(sd, strict=False)
    # Safety: reject EXP001 (pre-dualgraph) checkpoints that would leave
    # `gvp_encoder`, `h_res_proj`, `g_res_proj`, and the expanded
    # `specificity_header` first-layer RANDOMLY INITIALIZED (codex round 7).
    gvp_prefixes = ("gvp_encoder", "h_res_proj", "g_res_proj", "h_res_gate_logit")
    missing_gvp = [k for k in result.missing_keys if k.startswith(gvp_prefixes)]
    if missing_gvp:
        print(
            f"[ERROR] Checkpoint is missing {len(missing_gvp)} GVP parameter keys, "
            f"e.g. {missing_gvp[:3]}.\n"
            f"  This looks like a legacy EXP001/SS checkpoint, not an EXP005 "
            f"SSDualgraph one.\n"
            f"  Loading with strict=False would evaluate a partially random "
            f"model and give meaningless results — aborting.\n"
            f"  Pass an EXP005 SSDualgraph checkpoint to --test-only instead."
        )
        sys.exit(1)
    if result.missing_keys:
        print(
            f"[WARN] {len(result.missing_keys)} non-GVP keys missing from ckpt: "
            f"{result.missing_keys[:3]}"
        )
    if result.unexpected_keys:
        print(
            f"[WARN] {len(result.unexpected_keys)} unexpected keys in ckpt: "
    ...
    with torch.inference_mode():
        for batch in loader:
            ...
            all_logits.append(logits.squeeze(-1).detach().cpu().numpy())
            all_labels.append(batch.label.detach().cpu().numpy())
            tags = batch.tag if hasattr(batch, "tag") else ["0"] * len(batch.label)
    ...
    print(f"Inference    : {elapsed:.1f}s ({len(labels)/elapsed:.0f} samples/s)")
    print(f"{'='*50}")

    # Per-family
    per_family = {}
    print("\nPer-family AUC:")
    for fid in sorted(set(all_tags), key=lambda x: int(x) if x.isdigit() else x):
        mask = np.array([t == fid for t in all_tags])
        fl, fs = labels[mask], logits[mask]
        fname = FAMILY_NAMES.get(fid, fid)
        n, n_p = int(mask.sum()), int((fl == 1).sum())
        if len(np.unique(fl)) >= 2:
            fa, fp = float(roc_auc_score(fl, fs)), float(average_precision_score(fl, fs))
        else:
            fa, fp = None, None
        per_family[fid] = {"name": fname, "auc_roc": fa, "aupr": fp, "n_samples": n, "n_positive": n_p}
        auc_s = f"{fa:.4f}" if fa is not None else "N/A"
        print(f"  {fid} ({fname:12s}): AUC={auc_s}, n={n}, pos={n_p}")
    ...
    results = {
        ...
        "test_aupr": round(aupr, 6),
        "n_samples": len(labels), "n_positive": n_pos, "n_negative": n_neg,
...
def main():
    ...
    # Override dropout if specified via CLI
    if args.dropout is not None:
        if not 0.0 <= args.dropout <= 1.0:
            print(f'[ERROR] --dropout must be in [0, 1], got {args.dropout}')
            sys.exit(1)
        config.model.cross_attention.dropout = args.dropout

    # Test-only mode: evaluate and exit
    if args.test_only:
        test_evaluate(args, config)
        return
    ...
    # Output directories (flat, no nesting)
    CKPT_DIR = os.path.join(RESULTS_DIR, "checkpoints")
    os.makedirs(RESULTS_DIR, exist_ok=True)
    os.makedirs(CKPT_DIR, exist_ok=True)
    os.makedirs(LOG_DIR, exist_ok=True)

    # Validate config has the GVP block (codex round 6 — hard-fail, no defaults)
    if "gvp" not in config.model:
        print("[ERROR] EXP005 config must define `model.gvp`. "
              "Update configs/config.yml to add the gvp section.")
        sys.exit(1)
    ...
    for required in ["train", "val", "enzymes", "substrates"]:
        ...
        if not os.path.isdir(p):
            ...
            sys.exit(1)
    # EXP005: validate the GVP overlay sibling directory
    gvp_samples = os.path.join(os.path.dirname(cache_dir), "gvp_cache", "samples")
    if not os.path.isdir(gvp_samples):
        print(f"[ERROR] Missing GVP cache: {gvp_samples}")
        print(f"  Expected cache_dir points at overlay/random so its parent "
              f"is the overlay root containing gvp_cache/.")
        sys.exit(1)

    # Banner
    print("=" * 70)
    print("EZSpecificity Training (.pt shard cache)")
    print(f"  Edge mode   : {args.edge_mode}")
    ...
    # Model (EXP005: dualgraph 2+ SSDualgraph)
    print("[2/4] Initializing Model (random weights)...")
    model = SSDualgraph(config)
    model = _apply_runtime_patches(model)
    ...
    ckpt_cb = ModelCheckpoint(
        dirpath=CKPT_DIR,
        filename=f"pt-{run_tag}-ep{{epoch:02d}}-auc{{auc/val:.4f}}",
        monitor="auc/val", mode="max",
        save_top_k=5, save_last=True, verbose=True,
        auto_insert_metric_name=False,
    )
    early_stop_cb = EarlyStopping(
        monitor="auc/val", mode="max", patience=15, verbose=True,
    )
    run_metrics_csv = os.path.join(RESULTS_DIR, "metrics.csv")
    ...
    # EXP005: must use find_unused_parameters=True because the zero-init
    # strategy of h_res_proj + new header column causes gvp_encoder and
    # g_res_proj to receive zero gradient at step 0 (confirmed in L6 smoke
    # test). They self-unfreeze at step 1 once the output-side weights are
    # updated off zero, but DDP's default find_unused=False strict check
    # rejects any step where some params don't contribute to the loss.
    ...
    # Auto-test on best checkpoint
    best_ckpt = ckpt_cb.best_model_path
    if best_ckpt and os.path.exists(best_ckpt):
        print(f"\n[5/5] Auto-test on best checkpoint: {os.path.basename(best_ckpt)}")
        test_model = _apply_runtime_patches(
            SSDualgraph.load_from_checkpoint(best_ckpt, config=config)
        )
        SSDualgraph.transfer_batch_to_device = _fast_transfer  # re-apply
        trainer.test(model=test_model, datamodule=dm, verbose=False)

        if trainer.is_global_zero:
            import numpy as np
            logits = np.asarray(test_model.logits).ravel()
            labels = np.asarray(test_model.labels).ravel().astype(int)
            if len(np.unique(labels)) >= 2:
                auc = roc_auc_score(labels, logits)
                aupr = average_precision_score(labels, logits)
            else:
                auc, aupr = float("nan"), float("nan")
                print("[WARN] Test set has only one class, AUC/AUPR undefined.")
            n_pos = int((labels == 1).sum())
            n_neg = int((labels == 0).sum())

            # Per-family
            per_family = {}
            for key in sorted(test_model.labels_dict.keys()):
                fl = np.asarray(test_model.labels_dict[key]).ravel().astype(int)
                fs = np.asarray(test_model.logits_dict[key]).ravel()
                fname = FAMILY_NAMES.get(str(key), str(key))
                if len(np.unique(fl)) >= 2:
                    fa = float(roc_auc_score(fl, fs))
                    fp = float(average_precision_score(fl, fs))
                else:
                    fa, fp = None, None
                per_family[str(key)] = {
                    "name": fname, "auc_roc": fa, "aupr": fp,
                    "n_samples": len(fl), "n_positive": int((fl == 1).sum()),
                }

            test_json = os.path.join(RESULTS_DIR, "test_eval.json")
            results = {
        ...
        print("[WARN] Best checkpoint not found, skipping auto-test.")
