...
| Q2 | 序列相似度划分 | 🟢 **PathD 进行中**（EXP004-EXP010 已完成；下一步补做 EXP011 8:1:1） | 旧 PathC/EXP006 不继续；只作为历史参考 | P0 | 中 |
...
4. EXP010 的测试集更小，AUPR 只比 EXP005 略高；后续汇报必须同时展示 test samples 和 test positives。

下一步是 EXP011：使用 EXP009 已生成的 `strict_nn60_ratio811_rank01_seed0000` 候选划分，按 EXP010 的同一脚本和训练设置补做 8:1:1 版本。它主要用于判断 strict NN60 下继续提高训练集比例时，测试结果是否还会变化。
...
1. 补做 Q2 EXP011：strict NN60 8:1:1，训练设置沿用 EXP010。
2. 对比 EXP005、EXP010、EXP011，判断 strict NN60 下 train/val/test 比例变化对 Test AUC 和 Test AUPR 的影响。
3. 整理 Q2 的最终汇报图表，明确 random split、聚类簇划分、strict NN 阈值梯度和分布比例敏感性四条证据链。
4. Q2 结论稳定后，再启动 Q1 Fe 嵌入补丁对照。
