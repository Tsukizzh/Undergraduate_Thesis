...
| Q2 | 序列相似度划分 | 🟢 **PathD 进行中**（EXP004-EXP011 已完成；正在整理结论） | 旧 PathC/EXP006 不继续；只作为历史参考 | P0 | 中 |
...
| Q10 | 湿实验完整打分系统 | 🟡 已重新定义，可启动设计 | 已出现真实输入场景：diosgenin SMILES + 约 128 条 P450 序列；第二版必须重做结构、底物 3D、对接、模型缓存和最终排序 | P1 | 大 |
...
## Q2 当前进展（2026-05-25 更新）

Q2 已从早期数据准备阶段推进到正式训练结果对比阶段。现在可汇报的主线是：先证明 PathD 随机划分训练管线正常，再逐步增加序列相似度隔离强度，观察模型泛化能力如何变化。
...
1. PathD random split 的 Test AUC 与 PathC 历史 random baseline 接近，说明 PathD 的训练管线没有把随机划分基线跑坏。
2. strict NN40/60/80 形成清晰梯度，测试酶离训练酶越远，结果整体越低。
3. EXP010 在 strict NN60 约束不变的前提下提高训练集比例，Test AUC 从 EXP005 的 0.670801 回升到 0.733258，但仍低于 random split。
4. EXP011 继续提高训练集比例到 8:1:1，Test AUC 降到 0.602512。它的测试集只有 4466 条、435 个正样本，汇报时必须和样本规模一起解释。

EXP011 已完成：使用 EXP009 的 `strict_nn60_ratio811_rank01_seed0000` 候选划分，训练设置沿用 EXP010，最佳验证 AUC 出现在 epoch 9。结果显示，strict NN60 下继续把训练集比例提高到 8:1:1 并没有带来更好的测试结果。

## 四、推荐执行顺序（2026-05-25 更新）
...
Q9 先做底物类别下游预测，Q10 现在重新定义为湿实验完整打分系统。Q10 的第二版按完整流程执行：输入真实底物 SMILES 和候选 P450 序列后，必须完成 P450 结构重建、底物 3D 准备、重新对接、模型缓存重建、模型打分、最终优先级和湿实验结果回填表。网页或长期服务放到流程稳定后再考虑。
...
1. 整理 Q2 的最终汇报图表，明确 random split、聚类簇划分、strict NN 阈值梯度和分布比例敏感性四条证据链。
2. 对比 EXP005、EXP010、EXP011，说明 strict NN60 下比例变化的影响，同时标注每个测试集的样本数和正样本数。
3. Q2 结论稳定后，再启动 Q1 Fe 嵌入补丁对照。
...
| 2026-05-25 | Q2 已完成 EXP004-EXP010：包含 PathD random 对照、id60 聚类簇划分、strict NN40/60/80 阈值梯度和 strict NN60 7:1.5:1.5 分布敏感性训练 |
| 2026-05-25 | 下一步计划补做 EXP011：使用 EXP009 的 strict NN60 8:1:1 候选划分，训练脚本和参数沿用 EXP010 |
| 2026-05-25 | EXP011 已完成：strict NN60 8:1:1，best val AUC 0.840864，Test AUC 0.602512，Test AUPR 0.169727 |
