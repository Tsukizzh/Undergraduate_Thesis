...
"""
SOP生成脚本 - 04_配体三方深度验证
生成12个窗口的SOP文件，强制三方协作（Claude + Codex + Gemini）
"""

from pathlib import Path

PROJECT_ROOT = Path(__file__).resolve().parent.parent.parent.parent  # PathA_2026-01-08_模型评估测试集构建
OUTPUT_DIR = PROJECT_ROOT / "scripts/03_Step3_配体处理与数据集构建/04_配体三方深度验证"

# SOP模板
SOP_TEMPLATE = """# Chunk {chunk_id} - 配体三方深度验证SOP

## 核心任务

你负责验证 **Chunk {chunk_id}** 的 **{num_records}条** P450酶-配体记录。
...
**本chunk总时间**：{num_records}条 × 7分钟 ≈ {estimated_hours}小时
...
确认理解所有要求后，开始处理{num_records}条记录。
...
def generate_sops():
    ...
    # 读取manifest获取每个chunk的记录数
    import json
    manifest_file = PROJECT_ROOT / "data/03_Step3_配体处理与数据集构建/04_配体三方深度验证/01_输入数据_chunks/manifest.json"
    with open(manifest_file, 'r', encoding='utf-8') as f:
        manifest = json.load(f)

    for chunk_info in manifest["chunks"]:
        chunk_id = chunk_info["chunk_id"]
        num_records = chunk_info["num_records"]
        estimated_hours = round(num_records * 7 / 60, 1)

        # 文件路径（使用绝对路径）
        project_root = str(PROJECT_ROOT).replace("\\", "/")
        input_file = f"{project_root}/data/03_Step3_配体处理与数据集构建/04_配体三方深度验证/01_输入数据_chunks/chunk_{chunk_id}.csv"
        output_jsonl = f"{project_root}/data/03_Step3_配体处理与数据集构建/04_配体三方深度验证/02_输出数据_results/chunk_{chunk_id}_results/verified_results.jsonl"
        scripts_dir = f"{project_root}/scripts/03_Step3_配体处理与数据集构建/04_配体三方深度验证/chunk_{chunk_id}_scripts"
        temp_dir = f"{project_root}/data/03_Step3_配体处理与数据集构建/04_配体三方深度验证/02_输出数据_results/chunk_{chunk_id}_results/temp"
        session_dir = f"{project_root}/sessions/03_Step3_配体处理与数据集构建/04_配体三方深度验证/chunk_{chunk_id}_session"

        # 生成SOP内容
        sop_content = SOP_TEMPLATE.format(
            chunk_id=chunk_id,
        ...
        # 保存SOP文件
        sop_file = OUTPUT_DIR / f"sop_chunk_{chunk_id}.md"
        with open(sop_file, 'w', encoding='utf-8') as f:
            f.write(sop_content)

        print(f"[OK] SOP {chunk_id}: {num_records}条记录, 预计{estimated_hours}小时 -> {sop_file.name}")

    print("\n" + "=" * 60)
    print("SOP生成完成！")
    print("=" * 60)
    print(f"输出目录：{OUTPUT_DIR}")
    print(f"生成文件：sop_chunk_01.md ~ sop_chunk_12.md")
