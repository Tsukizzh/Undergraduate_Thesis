...
def _apply_runtime_patches(model):
    ...
    # DDP gather for BOTH validation and test epoch_end
    def _wrap_epoch_end(orig_fn):
        def _ddp_epoch_end(outputs):
            import numpy as np
            import pickle
            import torch.distributed as dist
            if dist.is_initialized() and dist.get_world_size() > 1:
                # Each rank has its own subset of outputs
                ...
                orig_fn(merged_outputs)
            else:
                orig_fn(outputs)
        return _ddp_epoch_end

    model.validation_epoch_end = _wrap_epoch_end(model.validation_epoch_end)
    model.test_epoch_end = _wrap_epoch_end(model.test_epoch_end)
    return model
