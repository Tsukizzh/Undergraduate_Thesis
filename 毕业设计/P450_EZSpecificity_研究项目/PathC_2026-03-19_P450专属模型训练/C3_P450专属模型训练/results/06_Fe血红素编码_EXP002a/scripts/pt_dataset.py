...
PROTEIN_FEAT_DIM = len(PROTEIN_ELEMENTS) + PROTEIN_NUM_AA + 2  # +is_hetero
...
def rebuild_protein_x(
    ...
    prot_feat = torch.cat([elem_oh, aa_oh, bb, het], dim=-1)
...
class PtCacheDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx: int) -> StructureSequenceData:
        ...
        protein_x = rebuild_protein_x(
            g["protein_element"], g["protein_aa_type"], g["protein_is_backbone"], g["protein_is_hetero"], n_lig
        )
