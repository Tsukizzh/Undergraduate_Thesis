...
def test_evaluate(args, config):
    ...
    if args.output_json:
        ...
        out_json = os.path.join(RESULTS_DIR, run_tag, "test_eval.json")
...
def main():
    ...
    run_metrics_csv = os.path.join(RUN_DIR, f"metrics_{run_tag}.csv")
    metrics_csv_cb = MetricsCSVLogger(csv_path=run_metrics_csv)
    logger = TensorBoardLogger(save_dir=LOG_DIR, name=f"pt_{run_tag}")
