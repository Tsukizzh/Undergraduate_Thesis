...
# import pyximport  # removed for Python 3.12
...
class FeaturizeProteinAtom(object):
    ...
    def __init__(self):
        ...
        self.max_num_aa = 22
        self.residue_angle_dim = 6

    @property
    def feature_dim(self):
        return self.atomic_numbers.size(0) + self.max_num_aa + 2 + self.residue_angle_dim
    ...
    def __call__(self, data: StructureComplexData):
        ...
        residue_angle = getattr(data, "protein_residue_angle_feature", None)
        if residue_angle is None:
            residue_angle = torch.zeros(
                (element.size(0), self.residue_angle_dim),
                dtype=torch.float32, device=data.protein_element.device)
        else:
            residue_angle = residue_angle.float()
        x = torch.cat([element, amino_acid, is_backbone, is_hetero, residue_angle], dim=-1)
