...
# SRC_DIR: prefer experiment-local src/ over global src/
_EXP_ROOT = os.path.normpath(os.path.join(_SCRIPT_DIR, ".."))
_EXP_SRC = os.path.join(_EXP_ROOT, "src")
_PROJECT_ROOT = os.path.normpath(os.path.join(_SCRIPT_DIR, "..", "..", "..", "..", ".."))
_SRC_DIR = _EXP_SRC if os.path.isdir(_EXP_SRC) else os.path.join(_PROJECT_ROOT, "src")
...
# ---------------------------------------------------------------------------
PROTEIN_ATOMIC_NUMBERS = [1, 6, 7, 8, 16, 34, 26]      # H, C, N, O, S, Se, Fe
...
def extract_structure_sample(structure_data: Any,
    ...
    protein_is_het  = _to_np(_get(structure_data, "protein_is_hetero"))        # (N_prot,)
    protein_residue_geom = _to_np(_get(structure_data, "protein_residue_angle_feature"))  # (N_prot, 6)
    ...
    protein_is_het_u8   = protein_is_het.astype(np.uint8) if protein_is_het is not None else np.zeros_like(protein_is_bb_u8)
    if protein_residue_geom is None:
        protein_residue_geom_f16 = np.zeros((N_prot, 6), dtype=np.float16)
    else:
        protein_residue_geom = np.asarray(protein_residue_geom, dtype=np.float32)
        if protein_residue_geom.ndim == 1:
            protein_residue_geom = protein_residue_geom.reshape(-1, 6)
        if protein_residue_geom.shape[0] != N_prot:
            raise ValueError(f"protein_residue_angle_feature rows {protein_residue_geom.shape[0]} != N_prot {N_prot}")
        protein_residue_geom_f16 = protein_residue_geom[:, :6].astype(np.float16)
    ...
    return {
        ...
        "protein_is_hetero": protein_is_het_u8,     # uint8   (N_prot,)
        "protein_residue_geom": protein_residue_geom_f16,  # float16 (N_prot, 6)
...
def _process_single_shard(args_tuple):
    ...
    prot_pos_chunks, prot_elem_chunks, prot_aa_chunks, prot_bb_chunks, prot_het_chunks, prot_geom_chunks = [], [], [], [], [], []
    ...
    for row_in_shard, (idx, ds_id, enz_idx, sub_idx, dock_idx, label) in enumerate(
        ...
        prot_het_chunks.append(struct_arrays["protein_is_hetero"])
        prot_geom_chunks.append(struct_arrays["protein_residue_geom"])
    ...
    def _cat_2d_rows(chunks, dtype, width=4):
        if not chunks: return np.zeros((0, width), dtype=dtype)
        return np.concatenate([np.asarray(c, dtype=dtype) for c in chunks], axis=0)
    ...
    shard_data = {
        ...
        "protein_is_hetero": torch.from_numpy(_cat(prot_het_chunks, np.uint8)),
        "protein_residue_geom": torch.from_numpy(_cat_2d_rows(prot_geom_chunks, np.float16, 6)),
...
def write_manifest(output_dir: Path, config: dict, k: int) -> None:
    ...
    manifest = {
        ...
        "protein_residue_geom_dim": 6,
        "protein_residue_geom_names": ["sin_phi", "cos_phi", "sin_psi", "cos_psi", "sin_chi1", "cos_chi1"],
...
def self_test(output_dir: Path) -> None:
    ...
    protein_het     = shard["protein_is_hetero"][s_prot:e_prot].long()   # (N_prot,)
    protein_geom    = shard["protein_residue_geom"][s_prot:e_prot].float()  # (N_prot, 6)
    ...
    prot_aa_oh   = F.one_hot(protein_aa.clamp(0, 21), num_classes=22).float()       # (N_prot, 21)
    prot_bb_f    = protein_bb.float().unsqueeze(1)                                  # (N_prot, 1)
    prot_het_f   = protein_het.float().unsqueeze(1)                                  # (N_prot, 1)
    protein_atom_feature = torch.cat([prot_elem_oh, prot_aa_oh, prot_bb_f, prot_het_f, protein_geom], dim=1)  # (N_prot, 37)
    protein_x = torch.cat([
        torch.zeros(N_lig, protein_atom_feature.shape[1]),
        protein_atom_feature,
    ], dim=0)  # (N_total, 28)
    print(f"  protein_x shape:     {protein_x.shape}  (expected ({N_total}, {protein_x.shape[1]}))")
    ...
    assert protein_x.shape[0] == N_total,        f"protein_x shape mismatch: {protein_x.shape}"
        ...
        for shard_id in tqdm(shard_ids, desc=f"  [{split}] shards"):
            ...
            for i in rows:
                ...
                sample = {
                    ...
                    "protein_is_hetero": shard["protein_is_hetero"][ps:pe].clone(),
                    "protein_residue_geom": shard["protein_residue_geom"][ps:pe].clone(),
