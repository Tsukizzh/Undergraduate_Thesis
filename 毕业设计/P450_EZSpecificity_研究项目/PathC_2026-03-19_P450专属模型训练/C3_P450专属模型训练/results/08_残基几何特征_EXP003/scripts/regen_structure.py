#!/usr/bin/env python3
"""Regenerate structure_features_geom.lmdb with parallel parsing.

Uses the PATCHED PDBProtein from EXP002a (reads HEM HETATM, feature_dim=37).
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Regenerate structure_features_geom.lmdb with patched PDBProtein (Fe/HEM + phi/psi/chi1 angles)."
    )
    p.add_argument("--exp-src",
        default="~/autodl-tmp/EZSpecificity/PathC/P450/experiments/EXP003_residue_geometry/src")
    p.add_argument("--csv-path",
        default="~/autodl-tmp/EZSpecificity/PathC/P450/data/data.csv")
    p.add_argument("--pocket-dir",
        default="~/autodl-tmp/EZSpecificity/PathC/P450/data/structure/str_tmp_data/pocket")
    p.add_argument("--ligand-dir",
        default="~/autodl-tmp/EZSpecificity/PathC/P450/data/structure/str_tmp_data/ligand")
    p.add_argument("--output-lmdb",
        default="~/autodl-tmp/EZSpecificity/PathC/P450/data/structure/structure_features_geom.lmdb")
...
def main():
    ...
    assert fdim == 37, f"Expected feature_dim=37 (patched), got {fdim}. Wrong src?"
