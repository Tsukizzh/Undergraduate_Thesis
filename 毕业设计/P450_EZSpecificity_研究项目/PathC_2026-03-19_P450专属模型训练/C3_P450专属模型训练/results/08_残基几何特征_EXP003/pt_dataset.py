...
PROTEIN_ELEMENTS = [1, 6, 7, 8, 16, 34, 26]  # +Fe
PROTEIN_NUM_AA = 22  # +HEM
PROTEIN_GEOM_DIM = 6  # sin/cos(phi, psi, chi1)
PROTEIN_FEAT_DIM = len(PROTEIN_ELEMENTS) + PROTEIN_NUM_AA + 2 + PROTEIN_GEOM_DIM
...
def rebuild_protein_x(
    ...
    protein_is_hetero: torch.Tensor,
    protein_residue_geom: torch.Tensor,
    n_lig: int,
) -> torch.Tensor:
    ...
    het = protein_is_hetero.float().unsqueeze(-1)
    prot_geom = protein_residue_geom.float()
    prot_feat = torch.cat([elem_oh, aa_oh, bb, het, prot_geom], dim=-1)
...
class PtCacheDataset(torch.utils.data.Dataset):
    ...
    def _load_graph_sample(self, idx: int) -> dict:
        ...
        if self._preloaded is not None and sample_id in self._preloaded:
            ...
            return {
                ...
                "protein_is_hetero": s["protein_is_hetero"].long(),
                "protein_residue_geom": s.get(
                    "protein_residue_geom",
                    torch.zeros((s["protein_element"].shape[0], PROTEIN_GEOM_DIM), dtype=torch.float32),
                ).float(),
        ...
        return {
            ...
            "protein_is_hetero": shard["protein_is_hetero"][s_p:e_p].long(),
            "protein_residue_geom": (
                shard["protein_residue_geom"][s_p:e_p].float()
                if "protein_residue_geom" in shard
                else torch.zeros((e_p - s_p, PROTEIN_GEOM_DIM), dtype=torch.float32)
            ),
    ...
    def __getitem__(self, idx: int) -> StructureSequenceData:
        ...
        protein_x = rebuild_protein_x(
            g["protein_element"],
            g["protein_aa_type"],
            g["protein_is_backbone"],
            g["protein_is_hetero"],
            g["protein_residue_geom"],
            n_lig
        )
