...
\textbf{P450Rdb}（成都中医药大学）。该数据库提供以反应为中心的 P450 条目。本研究从官方网站下载全部四个数据文件（其中 \texttt{Reactions.csv} 约 84~MB），识别出 132{,}000 余条原始记录实际上由两张子表拼接而成，遂仅保留其中标记为 Experimentally validated 的 3{,}753 条实验验证条目。对条目中合并写入的多个 UniProt 编号进行拆分，并对序列信息缺失的条目通过 UniProt\cite{UniProt2025} 序列 API 与序列 SHA 哈希桥接补全（共为 7 条记录回填序列）。该来源最终贡献 2{,}798 条正样本、857 个唯一酶与 1{,}492 个唯一底物。

\textbf{PlantP450DB}（哥本哈根大学植物 P450 数据库）。该数据库聚焦植物细胞色素 P450，其命名体系由 Nelson 等的植物 P450 系统命名工作奠定\cite{Nelson2004}。本研究自该数据库爬取 910 个酶条目详情页，通过 PubMed 与 CrossRef 检索获取相关文献 566 篇摘要，对开放获取的 52 篇文献通过 Unpaywall 下载全文，并补充 61 篇非开放获取文献的 PDF，在对应摘要与全文中自动化抽取酶--底物对关系、并经多轮质量复核；底物 SMILES 通过 PubChem 与 KEGG 的化学标识符交叉查询补齐，酶序列通过 UniProt 补齐。该来源最终贡献 979 条正样本、578 个唯一酶与 295 个唯一底物。

\textbf{PCPD}（中科院天津植物 P450 综合数据库）\cite{Wang2021PCPD}。该数据库通过前端 JavaScript 动态加载记录。本研究通过浏览器开发者工具逆向分析定位其静态资源 CDN，批量下载全部 1{,}427 条酶 JSON、1{,}425 条 FASTA 序列、423 个 PDB 结构与 857 张反应示意图片，通过 7 条正则规则与多轮人工复核提取 582 条表达不清晰的反应条目，并对初次抽取结果中 228 处字段错误做针对性清洗；底物 SMILES 通过三轮 PubChem 与 KEGG 查询补齐。该来源最终贡献 1{,}209 条正样本、818 个唯一酶与 570 个唯一底物。
...
\begin{figure}[htbp]
  \centering
  \fbox{\parbox[c][3.5cm][c]{0.85\textwidth}{\centering 图 3-1 占位：五源数据整合流程示意图\\[0.5em] （原始源 $\to$ 字段标准化 $\to$ 去重合并 $\to$ 正样本对 $\to$ 负样本与切分）\\[0.5em] 手绘待插入：\texttt{image/fig3-1-multisource.png}}}
  \caption{五源 P450 数据整合流程示意}
...
\begin{figure}[htbp]
  \centering
  \fbox{\parbox[c][3.5cm][c]{0.85\textwidth}{\centering 图 3-2 占位：RCSB 五级漏斗筛选示意\\[0.5em] 1{,}591 $\to$ 1{,}426 $\to$ 1{,}370 $\to$ 930 $\to$ 740 $\to$ 682\\[0.5em] 手绘待插入：\texttt{image/fig3-2-rcsb-funnel.png}}}
  \caption{RCSB PDB 来源五级漏斗筛选示意图}
...
不同数据源在酶标识、序列长度、底物 SMILES 规范化程度等字段上存在差异。为保证合并后的基准数据集内酶与底物的身份唯一性，本研究采用分层去重策略：在酶的层面，以 UniProt 登录号为主键，对缺失 UniProt 的条目通过酶序列的 SHA 哈希与 UniProt 序列 API 双向桥接后再行合并；在底物的层面，统一以 IUPAC 国际化学标识符 InChIKey\cite{InChI2015} 作为唯一键，先将源条目中的 SMILES 表达式\cite{SMILES1988} 经 RDKit\cite{RDKit} 规范化为标准 SMILES 与 InChI，随后以 InChIKey 作为去重键；对同一 InChIKey 对应多条 SMILES 的情况，保留最短规范化 SMILES 作为代表，其余保留为同义项以便后续回溯。
...
\begin{figure}[htbp]
  \centering
  \fbox{\parbox[c][3.5cm][c]{0.85\textwidth}{\centering 图 3-3 占位：外部过滤测试集构造流程\\[0.5em] 全 test 10{,}999 $\to$ 剔除与 ESIBank 重合的 P450 酶 $\to$ 保留 7{,}963 条\\[0.5em] 手绘待插入：\texttt{image/fig3-3-external-filter.png}}}
  \caption{泄漏控制外部公平对比测试集构造}
  \label{fig:ch3-filter}
\end{figure}
...
\textbf{对接流程}。采用 Uni-Dock\cite{UniDock2023} 作为对接引擎，该引擎基于 AutoDock Vina 2\cite{Vina2010,Vina2021} 评分函数并经 GPU 并行化大幅提速。对接盒按活性位点中心（以血红素铁为参考点）外扩 $20~\mathrm{\AA}$ 设置，采用快速模式以兼顾规模与精度；对每条酶--底物配对保留最佳打分对接姿态。
