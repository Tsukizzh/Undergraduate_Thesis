...
\begin{figure}[htbp]
  \centering
  \fbox{\parbox[c][5cm][c]{0.85\textwidth}{\centering \textbf{图 4-1 占位：泄漏控制外部公平对比可视化}\\[0.5em] 左面板：四条柱对比 AUC--AUPR\\ （参考 ckpt 未过滤 / 过滤后 \,vs\, 本研究 M0 未过滤 / 过滤后）\\[0.3em] 右面板：ROC + PR 曲线（过滤后测试集，参考 ckpt vs M0）\\[0.3em] 用以同时展示性能差距与稀有类打分塌缩\\[0.5em] 待插入：\texttt{image/fig4-1-fair-comparison.png}（matplotlib 自动生成）}}
  \caption{泄漏控制条件下参考 ckpt 与本研究 M0 的公平对比：柱状指标 + ROC/PR 曲线}
  \label{fig:ch4-fair}
\end{figure}
